...
```{r code=readLines("new_code/clean_v1_1_extract_chromosomes.R")}
...
```{r code=readLines("new_code/clean_v1_2_combine_feathers.R")}
...
readRDS("new_data/Xdat_common_chr1_subset_Blood.rds") -> x #%>% as.data.frame %>%
   dplyr::select(APL) %>% table()
x[,c("AGE","MALE")]
...
The following two functions compute the p-values and dmr regions.

```{r code=readLines("new_code/analyze_1_compute_p_values.R")}

```


```{r code=readLines("new_code/analyze_2_compute_dmr.R")}
...
<!--  knitr::include_graphics("./graphs/combined_alpha_.05_MALE.jpg", error=F) -->
<!-- ``` -->


<!-- ```{r eval=TRUE, echo = FALSE, out.width="100%"} -->
<!--  knitr::include_graphics("./graphs/combined_alpha_.1_MALE.jpg", error=F) -->
<!-- ``` -->


<!-- ```{r eval=TRUE, echo = FALSE, out.width="100%"} -->
<!--  knitr::include_graphics("./graphs/combined_alpha_.2_MALE.jpg", error=F) -->
<!-- ``` -->
...
The following function fits the 3 models (LMCC, GASP, Naive) on scenarios 1, 3, and 4 from Melina's paper.


```{r code=readLines("new_code/analyze_3_fit_models.R")}
...
> check the results document

#----------------------

### Putting it all together:

Run the file "new_code/main.R" to reproduce the same results.



```{r code=readLines("new_code/load_files.R")}

```


```{r code=readLines("new_code/main.R")}
