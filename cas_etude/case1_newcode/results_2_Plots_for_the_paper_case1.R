...
set.seed(2022)
readRDS("new_data/sample_info.rds") %>%
   ...
   transmute(
      ...
      CELL_TYPE = mapply(function(x)
         stringr::str_trunc(x, 31), as.character(CELL_TYPE))
   ) ->
...
tab1 <- xtable(
   dat.tab1,
   caption = paste0(
      "Overview of a sample of the donor information with ",
      "selected columns"
   ),
   label = "case2:tab1"
)
...
x %>% ggplot(aes(x = as.factor(AGE))) +
   ...
   ggtitle("Distribution of age among selected samples") -> p1
p1
