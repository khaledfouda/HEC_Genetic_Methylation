compute_p_values_and_plot_multivar <-
 function(chromosome,
          load_p_values = TRUE,
          Male.Only = TRUE,
          plot = TRUE,
  ...
  if (load_p_values == TRUE) {
   ...
   stopCluster(cl)
   saveRDS(p_values,
           paste0("new_data/p_values_Mutlivar_", chromosome, note, ".rds"))
  }
  ...
  if (plot == TRUE) {
   ...
   for (i in 1:2) {
    filename = paste0(
     "./graphs/manhattan_plot_Multivar_",
     i,
     "_",
     chromosome,
     "_alpha_",
    ...
    manh.dat %>%
     ...
     ylab("-log10(P-value)") +
     geom_hline(
      yintercept = -log10(corrected_alpha),
      linetype = "dashed",
      color = "red"
     ) +
     geom_hline(
