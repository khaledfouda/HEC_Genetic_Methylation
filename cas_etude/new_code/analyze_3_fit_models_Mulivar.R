run_model_on_chromosome_Multivar <- function(chromosome, Age.Only=TRUE, Male.Only=TRUE,alpha=.05,
  ...
  dmr_regions1 = get_dmr_regions(p_values[,1], sites, corrected_alpha, floor_by = floor_by,
                                min_freq = min_freq, middle_point = middle_point, return_seq = T)
  dmr_regions2 = get_dmr_regions(p_values[,2], sites, corrected_alpha, floor_by = floor_by,
                                 min_freq = min_freq, middle_point = middle_point, return_seq = T)
  print("Starting..")
  
  dmr_regions = union(dmr_regions1, dmr_regions2)
  print("Starting..")
  ...
  row.names(res) <- NULL
  print(res)
  save(res, file = paste0("results/res_multivar_",chromosome,"_AgeOnly_",
