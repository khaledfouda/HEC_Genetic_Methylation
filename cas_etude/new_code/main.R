...
for(chr in chromosomes)
   combine_feathers_to_rds(chromosome = chr)
# OR for dmrseq
...
for(chr in chromosomes){
   for(alpha in alphas)
      compute_p_values_and_plot(chr, load_p_values = FALSE, alpha=alpha,
                                min_freq=1, middle_point=TRUE, floor_by=1e6, plot=TRUE)
}
