...
for ( i in 1:4){
  ...
  if(discrete){
    scatter_4(
      ...
      save_pdf = save_pdf,
      path = paste(c("figures/proc_obs_",name_fig,".eps"),collapse = ""),
      width = 10,
      height = 6,
...
if(fig_time){
  load(file = "../data_res/res_discrete.Rdata")
  tab_time = as.vector(sapply(res, function(x) x$time[1:2]))
  load(file = "../data_res/res_continuous.Rdata")
  tab_time = c(tab_time,as.vector(sapply(res, function(x) x$time[1:2])))
  load(file = "../data_res/res_discrete_mispe.Rdata")
  tab_time = c(tab_time,as.vector(sapply(res, function(x) x$time[1:2])))
  load(file = "../data_res/res_discrete_mispe_block.Rdata")
  tab_time = c(tab_time,as.vector(sapply(res, function(x) x$time[1:2])))
  ...
  barplot_1(
    tab_time,
    save_pdf = save_pdf,
    path = paste(c("figures/time.eps"), collapse = ""),
    width = 10,
    height = 4.5,
